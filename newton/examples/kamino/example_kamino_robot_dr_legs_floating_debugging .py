...
def get_builder(
    model_version: Literal["v1", "v2", "v3", "v2_converted"], fixed_model: bool = False
) -> newton.ModelBuilder:
    ...
    short_body_labels = [label.split("/")[-1] for label in robot_builder.body_label]
    bodies_to_remove = [
        label
        for label, short_label in zip(robot_builder.body_label, short_body_labels, strict=True)
        if short_label.endswith("_l")
        or "_l_" in short_label
        or "_r_o" in short_label
        or "upperleg_rod" in short_label
        or "servohorn" in short_label
    ]
    remove_bodies_from_builder(robot_builder, bodies_to_remove)

    if fixed_model:
        for i in range(1, len(robot_builder.joint_type)):
            robot_builder.joint_type[i] = newton.JointType.FIXED

        robot_builder.joint_dof_count = 6
        robot_builder.joint_coord_count = 7
        robot_builder.joint_constraint_count = robot_builder.joint_count * 6

        robot_builder.joint_q = robot_builder.joint_q[: robot_builder.joint_coord_count]
        robot_builder.joint_target_q = robot_builder.joint_q[: robot_builder.joint_coord_count]

        robot_builder.joint_qd = robot_builder.joint_qd[: robot_builder.joint_dof_count]
        robot_builder.joint_f = robot_builder.joint_f[: robot_builder.joint_dof_count]
        robot_builder.joint_act = robot_builder.joint_act[: robot_builder.joint_dof_count]

        robot_builder.joint_axis = robot_builder.joint_axis[: robot_builder.joint_dof_count]
        robot_builder.joint_armature = robot_builder.joint_armature[: robot_builder.joint_dof_count]
        robot_builder.joint_target_ke = robot_builder.joint_target_ke[: robot_builder.joint_dof_count]
        robot_builder.joint_target_kd = robot_builder.joint_target_kd[: robot_builder.joint_dof_count]
        robot_builder.joint_damping = robot_builder.joint_damping[: robot_builder.joint_dof_count]
        robot_builder.joint_limit_lower = robot_builder.joint_limit_lower[: robot_builder.joint_dof_count]
        robot_builder.joint_limit_upper = robot_builder.joint_limit_upper[: robot_builder.joint_dof_count]
        robot_builder.joint_limit_ke = robot_builder.joint_limit_ke[: robot_builder.joint_dof_count]
        robot_builder.joint_limit_kd = robot_builder.joint_limit_kd[: robot_builder.joint_dof_count]
        robot_builder.joint_target_qd = robot_builder.joint_target_qd[: robot_builder.joint_dof_count]
        robot_builder.joint_effort_limit = robot_builder.joint_effort_limit[: robot_builder.joint_dof_count]
        robot_builder.joint_velocity_limit = robot_builder.joint_velocity_limit[: robot_builder.joint_dof_count]
        robot_builder.joint_friction = robot_builder.joint_friction[: robot_builder.joint_dof_count]
        robot_builder.joint_target_mode = robot_builder.joint_target_mode[: robot_builder.joint_dof_count]

        robot_builder.joint_cts = [0.0] * robot_builder.joint_constraint_count
...
class Example:
    def __init__(self, viewer: newton.viewer.ViewerBase, args=None):
        ...
        robot_builder = get_builder(args.model, args.fixed_model)
        ...
        self.model.joint_limit_lower.fill_(-newton.MAXVAL)
        self.model.joint_limit_upper.fill_(newton.MAXVAL)

        self.model.gravity.fill_(0.0)

        # Set initial velocities
        lin_vel_global = np.array([0.0, 0.0, 0.0])
        ang_vel_global = np.array([1.0, 1.0, 1.0]) * 5.0
        ...
        self.config.use_collision_detector = None
        ...
        self.config.padmm.rho_0 = 0.1
        self.config.padmm.use_graph_conditionals = getattr(args, "use_graph_conditionals", True) if args else True
        self.config.padmm.use_acceleration = False
        self.config.constraint_jacobian_method = "analytic"
